...
docker logs --tail=50 fluxbus-backend-ci | grep -i -E "(login|auth|error|exception|500)" | tail -30
...
docker exec fluxbus-backend-ci curl -s http://localhost:8081/api/health | jq . || docker exec fluxbus-backend-ci curl -s http://localhost:8081/api/health

echo ""
echo "📋 4. Verificando logs completos do backend (últimas 100 linhas)..."
docker logs --tail=100 fluxbus-backend-ci | tail -50
