...
public class BankService {
    ...
    public Page<BankDTO> getAllBanks(Pageable pageable) {
        log.info("Buscando bancos com paginaÃ§Ã£o");
    ...
    public BankDTO getBankById(UUID id) {
        ...
        Bank bank = bankRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Banco nÃ£o encontrado com ID: " + id));
        return BankDTO.fromEntity(bank);
    }
    
    @Transactional(readOnly = true)
    public BankDTO getBankByCode(String code) {
        log.info("Buscando banco por cÃ³digo: {}", code);
        Bank bank = bankRepository.findByCode(code)
                .orElseThrow(() -> new RuntimeException("Banco nÃ£o encontrado com cÃ³digo: " + code));
        return BankDTO.fromEntity(bank);
    }
    ...
    public BankDTO createBank(BankDTO dto) {
        ...
        // Verificar se jÃ¡ existe banco com mesmo cÃ³digo
        if (bankRepository.findByCode(dto.getCode()).isPresent()) {
            throw new RuntimeException("JÃ¡ existe um banco com o cÃ³digo: " + dto.getCode());
        }
        
        // Verificar se jÃ¡ existe banco com mesmo CNPJ
        if (dto.getCnpj() != null && !dto.getCnpj().isEmpty() && 
            bankRepository.findByCnpj(dto.getCnpj()).isPresent()) {
            throw new RuntimeException("JÃ¡ existe um banco com o CNPJ: " + dto.getCnpj());
        }
    ...
    public BankDTO updateBank(UUID id, BankDTO dto) {
        ...
        // Verificar se cÃ³digo jÃ¡ existe em outro banco
        if (!bank.getCode().equals(dto.getCode()) && 
            bankRepository.findByCode(dto.getCode()).isPresent()) {
            throw new RuntimeException("JÃ¡ existe um banco com o cÃ³digo: " + dto.getCode());
        }
        
        // Verificar se CNPJ jÃ¡ existe em outro banco
        if (dto.getCnpj() != null && !dto.getCnpj().isEmpty() && 
    ...
    public void deleteBank(UUID id) {
        ...
        // Verificar se hÃ¡ agÃªncias associadas
        // TODO: Implementar verificaÃ§Ã£o de agÃªncias associadas
    ...
    public List<BankDTO> searchBanksByCode(String code) {
        log.info("Buscando bancos por cÃ³digo: {}", code);
