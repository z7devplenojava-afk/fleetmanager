...
/**
 * DataLoader para popular o banco de dados com EPIs de teste
 * Executa apenas se nÃ£o houver EPIs cadastrados
 */
@Configuration
public class EPIDataLoader {
        private static final Logger log = LoggerFactory.getLogger(EPIDataLoader.class);

        private final PersonalProtectiveEquipmentRepository epiRepository;

        public EPIDataLoader(PersonalProtectiveEquipmentRepository epiRepository) {
                this.epiRepository = epiRepository;
        }

        @Bean
        @Profile("!prod") // NÃ£o executa em produÃ§Ã£o
        public CommandLineRunner initEPIData() {
                return args -> {
                        // Verificar se jÃ¡ existem EPIs no banco
                        long count = epiRepository.count();
                        if (count > 0) {
                                log.info("âœ… JÃ¡ existem {} EPIs no banco de dados. Pulando inicializaÃ§Ã£o de dados de teste.",
                                                count);
                                return;
                        }

                        log.info("ðŸ“¦ Inicializando dados de teste de EPIs...");
                        ...
                        PersonalProtectiveEquipment luvas = PersonalProtectiveEquipment.builder()
                                        ...
                                        .caNumber("EN 388")
                                        .caValidity(null) // Sem validade especÃ­fica
                                        .manufacturer("Ansell")
                                        .model("HyFlex 11-800")
                                        .unitOfMeasurement("UNIDADE")
                                        .minimumStock(50)
                        ...
                        log.info("âœ… Dados de teste de EPIs inicializados com sucesso! {} EPIs criados.", 5);
                };
        }
}
