-- Script para exportar todos os funcionários do banco
-- Execute este script no PostgreSQL para gerar os INSERTs

-- Copie o resultado deste SELECT e cole na migration V371__seed_all_employees.sql
...
SELECT 
    ...
    'cnh_number, cnh_expiration_date, cnh_category, ctps, ctps_series, ' ||
    'ctps_issue_date, ctps_issuing_agency, ctps_rural, titulo_eleitor, ' ||
    'titulo_eleitor_zona, titulo_eleitor_secao, carteira_identidade_orgao_emissor, ' ||
    'carteira_identidade_data_emissao, certificado_militar, cbo, pis, salario, ' ||
    'salario_por_extenso, periodo_pagamento, horario_trabalho, folga_semanal, ' ||
    'fgts_optante, fgts_data_opcao, fgts_banco_depositario, fgts_data_retratacao, ' ||
    'pis_data_cadastro, pis_banco_depositario, pis_endereco_banco, pis_codigo_banco, ' ||
    'pis_codigo_agencia, nome_pai, nome_mae, local_nascimento, grau_instrucao, ' ||
    'carteira_modelo_19, registro_geral_estrangeiro, casado_brasileiro, ' ||
    'nome_conjuge_estrangeiro, spouse_name, spouse_cpf, spouse_rg, spouse_birth_date, ' ||
    'spouse_phone, spouse_email, tem_filhos_brasileiros, quantidade_filhos_brasileiros, ' ||
    'data_chegada_brasil, naturalizado, decreto_naturalizacao, assinatura_funcionario, ' ||
    'data_rescisao, whatsapp, created_at, updated_at' ||
    ') VALUES (' ||
    COALESCE('''' || id || '''', 'NULL') || ', ' ||
    ...
    COALESCE('''' || REPLACE(document, '''', '''''') || '''', 'NULL') || ', ' ||
